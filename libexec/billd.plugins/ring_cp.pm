...
#           print $u_pinfo->{CELL_PHONE};
          my $phone = $u_pinfo->{CELL_PHONE};
          ...
          if ($user->{status} == 0) {
            ...
            $response = $astman->action(
              {
                Action      => 'Originate',
                Channel     => "Local/$conf{ASTERISK_CALL_PREF_CPHONE}$phone\@$conf{ASTERISK_CALL_CPHONE}",  # for CELL_PHONE (VASGOR)
#                Channel    => "SIP/$phone",   # for ASTERISK ext (VASGOR)
				...
				Timeout	    => "110000",
                Callerid    => "$conf{ASTERISK_CALLERID}", # custom variable (VASGOR)
