...
<script>
  ...
  function defaultRow(type, attr = {}) {
    ...
    jQuery(types_select).on('change', function (e) {
      ...
      if (action.fields !== undefined) {
        ...
        const allNamesAreValues = attr.value && action.fields.length > 1 && action.fields.every(item => item.name === 'value');

        let multiValues = allNamesAreValues ? attr.value.split(';') : [];

        action.fields.forEach(field => {
          if (allNamesAreValues) attr.value = multiValues.shift() || '';
  ...
  function addField(fields, parent, type, attr = {}) {
    ...
    if (fields.type === 'select') {
      let col = document.createElement('div');
      col.classList.add('col-md-6', 'mb-1');
    ...
    if (fields.type === 'datepicker') {
      let col = document.createElement('div');
      col.classList.add('col-md-6', 'mb-1');

      let field = document.createElement('input');
      field.classList.add('form-control', 'datepicker');
      field.placeholder = fields.placeholder;
      field.name = type;
      field.dataset.name = fields.name;
      if (attr.contains) field.value = attr.contains;
      if (attr.value) field.value = attr.value;

      let icon = document.createElement('i');
      icon.classList.add('fa', 'fa-calendar');

      let group = document.createElement('div');
      group.classList.add('input-group-text');
      group.appendChild(icon);

      let group_append = document.createElement('div');
      group_append.classList.add('input-group-append');
      group_append.appendChild(group);

      let input_group = document.createElement('div');
      input_group.classList.add('input-group');
      input_group.appendChild(field);
      input_group.appendChild(group_append);
  ...
  jQuery('#save-workflow').on('click', function () {
    ...
    jQuery('#actions').find('.action').each(function () {
      ...
      parent.find(`[name='${action.type}']`).each(function () {
        ...
        if (typeof action_value === 'undefined') return;
        if (Array.isArray(action_value)) action_value = action_value.join(',');

        action_data[jQuery(this).data('name')] = typeof action_data[jQuery(this).data('name')] !== 'undefined'
          ? `${action_data[jQuery(this).data('name')]};${action_value}` : action_value;
      });
      data.actions.push(action_data);
    })

    let workflow_id = jQuery('#WORKFLOW_ID').val();
    sendRequest('/api.cgi/crm/workflow' + (workflow_id ? `/${workflow_id}` : ''), data)
      .then((data) => {
        let message = data.errstr || '_{CRM_SAVED_SUCCESSFULLY}_'
