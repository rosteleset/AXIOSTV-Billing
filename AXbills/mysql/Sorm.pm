...
END AS STATUS,
#Abonent_addr
if((users_pi.zip > 0), users_pi.zip, '') AS ZIP,
...
DATE_ADD(users.last_update, INTERVAL -3 HOUR) AS users_updated,
DATE_ADD(users_pi.last_update, INTERVAL -3 HOUR) AS users_pi_updated,
DATE_ADD(internet_main.last_update, INTERVAL -3 HOUR) AS internet_main_updated,
if ((users.company_id > 0), DATE_ADD(companies.last_update, INTERVAL -3 HOUR), '0000-00-00 00:00:00') AS companies_updated
from users_pi
...
#Соберем в массив данные из базы скрипта
	$sth = $dbh->prepare(qq{SELECT ABONENT_ID,
		...
		DATE_ADD(CURRENT_TIMESTAMP, INTERVAL -3 HOUR) AS CURRENT_TIMESTAMP_3,
...
#Платежи из Абиллса
		...
		DATE_ADD(payments.date, INTERVAL -3 HOUR) AS PAYMENT_DATE,
	...
	foreach my $entry (@db_p_array)
	{
		if($entry->{axbills_id} && exists $axbills_p_array{$entry->{axbills_id}})
		{
			...
			#Ключ найден в базе скрипта, но не найден в Абиллсе, заносим в очередь на удаление, но только если UID существует в AXbills
        	        print Payments "35;83;$entry->{PAY_TYPE_ID};$entry->{PAYMENT_DATE};$entry->{AMOUNT};;$entry->{PHONE};$entry->{ACCOUNT};$entry->{ABONENT_ID};;;;;;;;;0;;;;;;;;;;0;0;$entry->{ZIP};$entry->{COUNTRY};$entry->{REGION};$entry->{ZONE};$entry->{CITY};$entry->{STREET};$entry->{BUILDING};;$entry->{APARTMENT};;2\n";
...
#        print Dumper \@new_p_entries;# if ($debug == '1');
        ...
        foreach my $new_entry (@new_p_entries)
        {
			...
			print Payments "35;83;$new_entry->{PAY_TYPE_ID};$new_entry->{PAYMENT_DATE};$new_entry->{AMOUNT};;$new_entry->{PHONE};$new_entry->{ACCOUNT};$new_entry->{ABONENT_ID};;;;;;;;;0;;;;;;;;;;0;0;$new_entry->{ZIP};$country;$region;$zone;$city;$street;$build;;$apart;;1\n";
...
#Из базы скрипта
	 ...
	 my $need_update_files = 0;
         ...
         printf "======IP_PLAN SECTION=======\n";

         my $sth = $dbh->prepare(qq{SELECT record_id, id, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL -3 HOUR) AS CURRENT, DESCRIPTION, IPV4_START, IPV4_END, BEGIN_TIME, END_TIME, CURRENT_TIMESTAMP FROM IP_PLAN ORDER BY IP_PLAN.record_id DESC});
...
#Обновления
        ...
        $sth = $dbh->prepare(qq{SELECT id, DESCRIPTION, IPV4_START, IPV4_END, BEGIN_TIME FROM IP_PLAN ORDER BY IP_PLAN.id DESC});

        $sth->execute or die DBI->errstr;

        while(my $new_data = $sth->fetchrow_hashref())
        {
                 $update_array{$new_data->{id}} = $new_data;
        }

#Ищем совпадения, выпиливаем их
        foreach my $entry (@db_array)
        {
                if ($entry->{id} && !exists $update_array{$entry->{id}} && $entry->{END_TIME} eq $date_ended)
                {
                        $need_update_files = 1;
                        #Deleting
                        my $sth = $dbh->prepare(qq{UPDATE IP_PLAN SET END_TIME = '$entry->{CURRENT}'
                                                        WHERE IP_PLAN.record_id = $entry->{record_id}});
                        $sth->execute() if ($debug == '0');
                }

                if($entry->{id} && exists $update_array{$entry->{id}})
                {
                         if ($update_array{$entry->{id}}->{id} eq $entry->{id})
                                {
                                	if ($update_array{$entry->{id}}->{BEGIN_TIME} ne $entry->{BEGIN_TIME})
					{
						if ($update_array{$entry->{id}}->{IPV4_START} ne $entry->{IPV4_START} ||
						$update_array{$entry->{id}}->{IPV4_END} ne $entry->{IPV4_END} ||
						$update_array{$entry->{id}}->{DESCRIPTION} ne $entry->{DESCRIPTION})
						{
                                                        $need_update_files = 1;

                                                        my $sth = $dbh->prepare(qq{UPDATE IP_PLAN SET END_TIME = '$update_array{$entry->{id}}->{BEGIN_TIME}'
                                                                                        WHERE IP_PLAN.record_id = $entry->{record_id}});
                                                        $sth->execute() if ($debug == '0');

                                                        $sth = $dbh->prepare(qq{INSERT INTO IP_PLAN (id, DESCRIPTION, IP_TYPE, IPV4_START, IPV4_END, BEGIN_TIME, END_TIME)
                                        			VALUES ('$entry->{id}', '$update_array{$entry->{id}}->{DESCRIPTION}', '0',
                                                			'$update_array{$entry->{id}}->{IPV4_START}', '$update_array{$entry->{id}}->{IPV4_END}', '$update_array{$entry->{id}}->{BEGIN_TIME}', '$date_ended');
                                                        });
                                                        $sth->execute() if ($debug == '0');
                                                        delete $update_array{$entry->{id}};
                                                }
                                        } else { delete $update_array{$entry->{id}} }
                                }

                }
	}
        @new_entries = values %update_array;
        ...
        foreach my $new_entry (@new_entries)
        {
                ...
                {
			$need_update_files = 1;
                        my $sth = $dbh->prepare(qq{INSERT INTO IP_PLAN (id, DESCRIPTION, IP_TYPE, IPV4_START, IPV4_END, BEGIN_TIME, END_TIME)
                                        VALUES ('$new_entry->{id}', '$new_entry->{DESCRIPTION}', '0',
                                                '$new_entry->{IPV4_START}', '$new_entry->{IPV4_END}', '$new_entry->{BEGIN_TIME}', '$date_ended');
                        });
                        $sth->execute() if ($debug == '0');
                }
        }
#	return;
ipplan_export() if ($need_update_files == 1);
}
...
#Export to Files
	 ...
	 print Ip_plan "DESCRIPTION;IP_TYPE;IPV4;IPV6;IPV4_MASK;IPV6_MASK;BEGIN_TIME;END_TIME;REGION_ID\n";
         my $sth = $dbh->prepare(qq{SELECT record_id, id, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL -3 HOUR) AS CURRENT, DESCRIPTION, BEGIN_TIME, END_TIME, IPV4_START, IPV4_END, IP_TYPE FROM IP_PLAN ORDER BY record_id ASC});
         $sth->execute or die DBI->errstr;
			       ...
			       for (;;) 
			       {
					if ((length($clean_hex_ip) < 8) && length($clean_hex_ip) > 4) 
					{
						$clean_hex_ip = "0$clean_hex_ip";
                               ...
                               print Ip_plan "$data->{DESCRIPTION};0;$clean_hex_ip;;$clean_hex_mask;;$data->{BEGIN_TIME};$data->{END_TIME};35\n";
                               }
         }
        return;
}
...
#Из базы скрипта
	 ...
	 my DBI $dbh = $self->{db}->{db};
         ...
         my $sth = $dbh->prepare(qq{SELECT record_id, DISABLE, GATE_ID AS id, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL -3 HOUR) AS CURRENT, BEGIN_TIME, END_TIME, DESCRIPTION, IPV4, IP_PORT, STREET, BUILDING, APARTMENT, ZIP, CITY FROM GATEWAY ORDER BY record_id DESC});
...
#AXbills
        ...
        $sth = $dbh->prepare(qq{SELECT id, disable AS DISABLE, DATE_ADD(changed, INTERVAL -3 HOUR) AS BEGIN_TIME, INET_NTOA(ip) AS IPV4, mng_host_port AS IP_PORT, 
				name AS DESCRIPTION, 
				address_street AS STREET, address_build AS BUILDING, address_flat AS APARTMENT, zip AS ZIP, city AS CITY 
				FROM nas WHERE nas_type='accel_ppp' ORDER BY id ASC});
...
#Ищем совпадения, выпиливаем их
                ...
                if ($entry->{id} && !exists $axbills_array{$entry->{id}} && $entry->{END_TIME} eq $date_ended)
		{
			$need_update_files = 1;
                        #Deleting
			my $sth = $dbh->prepare(qq{UPDATE GATEWAY SET END_TIME = '$entry->{CURRENT}'
                                                        WHERE GATEWAY.record_id = $entry->{record_id}});
                        $sth->execute() if ($debug == '0');
                }
                ...
                if($entry->{id} && exists $axbills_array{$entry->{id}})
                {
						...
						$axbills_array{$entry->{id}}->{IP_PORT} ne $entry->{IP_PORT})
							...
							my $sth = $dbh->prepare(qq{UPDATE GATEWAY SET END_TIME = '$axbills_array{$entry->{id}}->{BEGIN_TIME}' 
											WHERE GATEWAY.record_id = $entry->{record_id}});
			 				$sth->execute() if ($debug == '0');

                         				$sth = $dbh->prepare(qq{INSERT INTO GATEWAY (GATE_ID, DISABLE, BEGIN_TIME, END_TIME, DESCRIPTION, IPV4, IP_PORT, ZIP, CITY, STREET, BUILDING, APARTMENT)
...
#        print Dumper \@new_entries;# if ($debug == '1');
	...
	#ADD Entries
		...
		next if (defined($new_entry->{record_id}));
			 $need_update_files = 1;
                         my $sth = $dbh->prepare(qq{INSERT INTO GATEWAY (GATE_ID, DISABLE, BEGIN_TIME, DESCRIPTION, IPV4, IP_PORT, ZIP, CITY, STREET, BUILDING, APARTMENT)
...
gateway_export() if ($need_update_files == 1);
}
...
sub gateway_export{
	 ...
	 my DBI $dbh = $self->{db}->{db};
#Export to Files
         my $sth = $dbh->prepare(qq{SELECT GATE_ID AS id, BEGIN_TIME, END_TIME, DESCRIPTION, IPV4, IP_PORT, STREET, BUILDING, APARTMENT, ZIP, CITY FROM GATEWAY ORDER BY record_id ASC});
         $sth->execute or die DBI->errstr;
	 ...
	 print Gateway_ip "GATE_ID;IP_TYPE;IPV4;IPV6;IP_PORT;REGION_ID\n";
		...
		my $hexport = uc(sprintf("%x", $port_to_hex[1]));
		...
		print Gateway "$data->{id};$data->{BEGIN_TIME};$data->{END_TIME};$data->{DESCRIPTION};7;0;0;$data->{ZIP};Российская федерация;Вологодская область;Бабаевский район;$data->{CITY};$data->{STREET};$data->{BUILDING};;$data->{APARTMENT};;35\n";
		print Gateway_ip "$data->{id};0;$clean_hex_ip;;$hexport;35\n";
         }
        return;
}
...
#Из базы скрипта
	 ...
	 my DBI $dbh = $self->{db}->{db};
         ...
         my $sth = $dbh->prepare(qq{SELECT record_id, ID AS id, BEGIN_TIME, END_TIME, DESCRIPTION, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL -3 HOUR) AS CURRENT 
					FROM SUPPLEMENTARY_SERVICE ORDER BY record_id DESC});
...
#AXbills
        ...
        $sth = $dbh->prepare(qq{SELECT id, DATE_ADD(last_update, INTERVAL -3 HOUR) AS BEGIN_TIME, name AS DESCRIPTION 
...
#Ищем совпадения, выпиливаем их
        foreach my $entry (@db_array)
        {
                if ($entry->{id} && !exists $axbills_array{$entry->{id}} && $entry->{END_TIME} eq $date_ended)
                {
                        $need_update_files = 1;
                        #Deleting
                        my $sth = $dbh->prepare(qq{UPDATE SUPPLEMENTARY_SERVICE SET END_TIME = $entry->{CURRENT}
                                                        WHERE SUPPLEMENTARY_SERVICE.record_id = $entry->{record_id}});
                        $sth->execute() if ($debug == '0');
                }
                         ...
                         if ($axbills_array{$entry->{id}}->{id} eq $entry->{id})
                                {
                                        if (($axbills_array{$entry->{id}}->{BEGIN_TIME} ne $entry->{BEGIN_TIME}))# && $entry->{END_TIME} eq $date_ended)
                                        {
                                                ...
                                                {
                                                        $need_update_files = 1;
                                                        my $sth = $dbh->prepare(qq{UPDATE SUPPLEMENTARY_SERVICE SET END_TIME = '$axbills_array{$entry->{id}}->{BEGIN_TIME}'
                                                                                        WHERE SUPPLEMENTARY_SERVICE.record_id = $entry->{record_id}});
                                                        $sth->execute() if ($debug == '0');

                                                        $sth = $dbh->prepare(qq{INSERT INTO SUPPLEMENTARY_SERVICE (ID, BEGIN_TIME, END_TIME, DESCRIPTION)
...
#        print Dumper \@new_entries;# if ($debug == '1');
        ...
        foreach my $new_entry (@new_entries)
        {
                ...
                {
                         $need_update_files = 1;
                         my $sth = $dbh->prepare(qq{INSERT INTO SUPPLEMENTARY_SERVICE (ID, BEGIN_TIME, DESCRIPTION, END_TIME)
...
supplementary_export() if ($need_update_files == 1);
}
...
sub supplementary_export{
	 ...
	 my DBI $dbh = $self->{db}->{db};
#Export to Files
         my $sth = $dbh->prepare(qq{SELECT ID AS id, BEGIN_TIME, END_TIME, DESCRIPTION FROM SUPPLEMENTARY_SERVICE ORDER BY record_id DESC});
         $sth->execute or die DBI->errstr;
         ...
         while(my $data = $sth->fetchrow_hashref())
         {
                print Supplementary "$data->{id};;$data->{BEGIN_TIME};$data->{END_TIME};$data->{DESCRIPTION};35\n";
         }
        return;
