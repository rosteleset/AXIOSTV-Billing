...
DETERMINISTIC
  BEGIN
    ...
    SELECT CONCAT(SUBSTRING(`actions`, 1, position('-' IN `actions`) - 1), ',', `datetime`)
    INTO `last_tp_info`

    FROM `admin_actions`
    WHERE `uid` = `user_id` AND `action_type` = 3
    ORDER BY `id` DESC
    LIMIT 1;
    RETURN `last_tp_info`;
  END|
...
DETERMINISTIC
  BEGIN
    ...
    SELECT CONCAT(`datetime`, ',', `actions`)
    INTO `action_info`
    FROM `admin_actions`
    WHERE `uid` = `user_id` AND `action_type` = `action_type_id` AND `module` = ''
    ORDER BY `id` DESC
    LIMIT 1;
